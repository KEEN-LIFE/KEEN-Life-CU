...
//
//*****************************************************************************
 unsigned char g_pcUARTTxBuffer2[UART_TX_BUFFER_SIZE2];
 volatile uint32_t g_ui32UARTTxWriteIndex2 = 0;
 volatile uint32_t g_ui32UARTTxReadIndex2 = 0;
...
//
//*****************************************************************************
 unsigned char g_pcUARTRxBuffer2[UART_RX_BUFFER_SIZE2];
 volatile uint32_t g_ui32UARTRxWriteIndex2 = 0;
 volatile uint32_t g_ui32UARTRxReadIndex2 = 0;
...
//
//*****************************************************************************
#define TX_BUFFER_USED2          (GetBufferCount2(&g_ui32UARTTxReadIndex2,  \
                                                &g_ui32UARTTxWriteIndex2, \
                                                UART_TX_BUFFER_SIZE2))
#define TX_BUFFER_FREE2          (UART_TX_BUFFER_SIZE2 - TX_BUFFER_USED2)
#define TX_BUFFER_EMPTY2         (IsBufferEmpty2(&g_ui32UARTTxReadIndex2,   \
                                               &g_ui32UARTTxWriteIndex2))
#define TX_BUFFER_FULL2          (IsBufferFull2(&g_ui32UARTTxReadIndex2,  \
                                              &g_ui32UARTTxWriteIndex2, \
                                              UART_TX_BUFFER_SIZE2))
#define ADVANCE_TX_BUFFER_INDEX2(Index) \
                                (Index) = ((Index) + 1) % UART_TX_BUFFER_SIZE2
...
//
//*****************************************************************************
#define RX_BUFFER_USED2          (GetBufferCount2(&g_ui32UARTRxReadIndex2,  \
                                                &g_ui32UARTRxWriteIndex2, \
                                                UART_RX_BUFFER_SIZE2))
#define RX_BUFFER_FREE2          (UART_RX_BUFFER_SIZE2 - RX_BUFFER_USED2)
#define RX_BUFFER_EMPTY2         (IsBufferEmpty2(&g_ui32UARTRxReadIndex2,   \
                                               &g_ui32UARTRxWriteIndex2))
#define RX_BUFFER_FULL2          (IsBufferFull2(&g_ui32UARTRxReadIndex2,  \
                                              &g_ui32UARTRxWriteIndex2, \
                                              UART_RX_BUFFER_SIZE2))
#define ADVANCE_RX_BUFFER_INDEX2(Index) \
                                (Index) = ((Index) + 1) % UART_RX_BUFFER_SIZE2
#endif
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTBase2[7] =
{
    UART0_BASE, UART1_BASE, UART2_BASE, UART3_BASE, UART4_BASE, UART5_BASE, UART6_BASE
};
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTInt2[7] =
{
    INT_UART0, INT_UART1, INT_UART2, INT_UART3, INT_UART4, INT_UART5, INT_UART6
};
...
//
//*****************************************************************************
static uint32_t g_ui32PortNum2;
#endif
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTPeriph2[7] =
{
...
#ifdef UART_BUFFERED2
static bool
IsBufferFull2(volatile uint32_t *pui32Read,
...
#ifdef UART_BUFFERED2
static bool
IsBufferEmpty2(volatile uint32_t *pui32Read,
...
#ifdef UART_BUFFERED2
static uint32_t
GetBufferCount2(volatile uint32_t *pui32Read,
               volatile uint32_t *pui32Write, uint32_t ui32Size)
{
    uint32_t ui32Write;
    uint32_t ui32Read;
...
#ifdef UART_BUFFERED2
static void
UARTPrimeTransmit2(uint32_t ui32Base)
{
    //
    ...
    if(!TX_BUFFER_EMPTY2)
    {
        ...
        //
        MAP_IntDisable(g_ui32UARTInt2[g_ui32PortNum2]);

        //
        // Yes - take some characters out of the transmit buffer and feed
        // them to the UART transmit FIFO.
        //
        while(MAP_UARTSpaceAvail(ui32Base) && !TX_BUFFER_EMPTY2)
        {
            MAP_UARTCharPutNonBlocking(ui32Base,
                                      g_pcUARTTxBuffer2[g_ui32UARTTxReadIndex2]);
            ADVANCE_TX_BUFFER_INDEX2(g_ui32UARTTxReadIndex2);
        }

        //
        // Reenable the UART interrupt.
        //
        MAP_IntEnable(g_ui32UARTInt2[g_ui32PortNum2]);
    }
}
#endif
...
//
//*****************************************************************************
void UARTStdioConfig2(uint32_t ui32PortNum, uint32_t ui32Baud, uint32_t ui32SrcClock)
{
    //
    // Check the arguments.
    //
    ASSERT((ui32PortNum == 0) || (ui32PortNum == 1) ||
           (ui32PortNum == 2) || (ui32PortNum == 7));
...
//    }
    ...
    //
    g_ui32Base2 = g_ui32UARTBase2[ui32PortNum];

    //
    // Enable the UART peripheral for use.
    //
    MAP_SysCtlPeripheralEnable(g_ui32UARTPeriph2[ui32PortNum]);
    ...
    //
    MAP_UARTFIFOLevelSet(g_ui32Base2, UART_FIFO_TX1_8, UART_FIFO_RX1_8);
    ...
    //
    MAP_UARTIntDisable(g_ui32Base2, 0xFFFFFFFF);
    MAP_UARTIntEnable(g_ui32Base2, UART_INT_RX | UART_INT_RT);
    MAP_IntEnable(g_ui32UARTInt2[ui32PortNum]);
#endif
...
//            if(!TX_BUFFER_FULL2)
//            {
//                g_pcUARTTxBuffer2[g_ui32UARTTxWriteIndex2] = '\r';
//                ADVANCE_TX_BUFFER_INDEX2(g_ui32UARTTxWriteIndex2);
//            }
//            else
//            {
//                //
...
//        }
        ...
        if(!TX_BUFFER_FULL2)
        {
            g_pcUARTTxBuffer2[g_ui32UARTTxWriteIndex2] = pcBuf[uIdx];
            ADVANCE_TX_BUFFER_INDEX2(g_ui32UARTTxWriteIndex2);
        }
        else
        {
    ...
    if(!TX_BUFFER_EMPTY2)
    {
        UARTPrimeTransmit2(g_ui32Base2);
        MAP_UARTIntEnable(g_ui32Base2, UART_INT_TX);
    }
...
//        if(!TX_BUFFER_FULL)
//        {
//            g_pcUARTTxBuffer[g_ui32UARTTxWriteIndex] = cBuf;
...
//int
//UARTwriteNonCommandLine(const char *pcBuf, uint32_t ui32Len)
//{
//#ifdef UART_BUFFERED
//    unsigned int uIdx;
...
UARTgetc2(void)
{
    ...
    //
    cChar = g_pcUARTRxBuffer2[g_ui32UARTRxReadIndex2];
    ADVANCE_RX_BUFFER_INDEX2(g_ui32UARTRxReadIndex2);
...
//*****************************************************************************
//void
//UARTprintf2(const char *pcString, ...)
//{
//    va_list vaArgP;
...
//
//*****************************************************************************
#if defined(UART_BUFFERED2) || defined(DOXYGEN)
int
UARTRxBytesAvail2(void)
{
    return(RX_BUFFER_USED2);
}
#endif

//#if defined(UART_BUFFERED) || defined(DOXYGEN)
//int
//UARTTxBytesAvail(void)
//{
...
//*****************************************************************************
int
UARTTxBytesFree2(void)
{
    return(TX_BUFFER_FREE2);
}
#endif
...
#if defined(UART_BUFFERED2) || defined(DOXYGEN)
int
UARTPeek2(unsigned char ucChar)
{
    int iCount;
    int iAvail;
    ...
    //
    iAvail = (int)RX_BUFFER_USED2;
    ui32ReadIndex = g_ui32UARTRxReadIndex2;
    ...
    {
        if(g_pcUARTRxBuffer2[ui32ReadIndex] == ucChar)
        {
            //
        ...
        else
        {
            ...
            //
            ADVANCE_RX_BUFFER_INDEX2(ui32ReadIndex);
        }
    }
...
UARTFlushRx2(void)
{
    ...
    //
    g_ui32UARTRxReadIndex2 = 0;
    g_ui32UARTRxWriteIndex2 = 0;
...
//*****************************************************************************
//
//! Flushes the receive buffer array completely and make both g_ui32UARTRxReadIndex, g_ui32UARTRxWriteIndex to 0.
//!
//! \return None.
//
//*****************************************************************************
...
{
	uint32_t ui32ReadIndex = 0,ui32WriteIndex = 0;
	
	for(ui32WriteIndex = 0;ui32WriteIndex<g_ui32UARTRxWriteIndex2;ui32WriteIndex++ )	
	{
		g_pcUARTRxBuffer2[ui32WriteIndex] = 0;
	}
...
UARTFlushTx2(bool bDiscard)
{
    ...
    if(bDiscard)
    {
        ...
        //
        g_ui32UARTTxReadIndex2 = 0;
        g_ui32UARTTxWriteIndex2 = 0;
...
UARTStdioIntHandler2(void)
{
    ...
    //
    ui32Ints = MAP_UARTIntStatus(g_ui32Base2, true);
    MAP_UARTIntClear(g_ui32Base2, ui32Ints);
    ...
    if(ui32Ints & UART_INT_TX)
    {
        ...
        if(TX_BUFFER_EMPTY2)
        {
            MAP_UARTIntDisable(g_ui32Base2, UART_INT_TX);
        }
    }
    ...
    if(ui32Ints & (UART_INT_RX | UART_INT_RT))
    {
        ...
        //
        while(MAP_UARTCharsAvail(g_ui32Base2))
        {
            //
            // Read a character
            //
            i32Char = MAP_UARTCharGetNonBlocking(g_ui32Base2);
...
/*					if(ulTcpIpSocketStatus != 3)
 ...
 //                   cChar = '\r';
            ...
            if(!RX_BUFFER_FULL2)
            {
                ...
                //
                g_pcUARTRxBuffer2[g_ui32UARTRxWriteIndex2] =
                    (unsigned char)(i32Char & 0xFF);
                ADVANCE_RX_BUFFER_INDEX2(g_ui32UARTRxWriteIndex2);
