...
//			 ctrlReadParam.Insp_Pressure = ctrlWriteRead.Set_Pmax;
				...
				ctrlWriteRead.Apnea_Time = 4;
				
/* changes  added for PCM mode checking*/
				
				if(ctrlWriteRead.Set_pressuresupport < 0)ctrlWriteRead.Set_Insp_Pressure = 0;
...
/* changes  added for PCM mode checking*/				
			...
			if(ulInhalationFlag == 1)
			{
			 ...
			 if(ventAlgParam.Peakflow <= ctrlServParam.Inh_Flow) //Finding Peak Flow
				...
				else if(ulInhalationCounter >= 4000) // for starting exhalation as safety mechanism
				{
