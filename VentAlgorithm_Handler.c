...
float PEEP[4]={0},AVG_PEEP=0,AVG_ERROR=0,SET_PEEP=0,AVG_PEEP_ACC=0,PEEP_BUFFER[50];

uint8_t  peep_f=0,patient_F=0,AVG_CNT=0;
...
void Vent_Algorithm(void)
{
	...
	VentMode = (!medProc_Flag)* (uint8_t)ctrlWriteRead.Vent_Mode; //this expression is for checking medical procedure
	 ...
	 if(VentMode == PRESSURE_CTRL_MODE)
		{
			...
			ctrlWriteParam.Set_RespRate =16;
			ctrlWriteParam.Set_RiseTime = 10; 
			ctrlWriteParam.Set_Insp_Pressure = 15; 
			ctrlWriteParam.Set_FI02 = 21;
			ctrlWriteParam.Set_InspTime=0;
			ctrlWriteParam.Set_IE_Ratio = 0.5;
			ctrlWriteParam.Flow_TrigSenstvty = 2;
			ctrlWriteParam.Pres_TrigSenstvty = 2;	
		...
		if(VentMode == PRESSURESUPPORT)
		{
			...
			ctrlWriteParam.Set_FI02 = 21;
			ctrlWriteParam.Flow_Exp_trigger=20;
			ctrlWriteParam.Flow_TrigSenstvty = 2;
			ctrlWriteParam.Pres_TrigSenstvty = 2;	
		...
		if(VentMode == SIMV)
		{
			...
			ctrlWriteParam.Set_TidalVol = 400;
			Volume_comp=ctrlWriteParam.Set_TidalVol ;
			ctrlWriteParam.Set_PauseTime = 1;			
			ctrlWriteParam.Set_Insp_Pressure = 15; 
			ctrlWriteParam.Set_pressuresupport=10;
			ctrlWriteParam.Flow_Exp_trigger=20;
		...
		if(VentMode == NIV)
		{
			ctrlWriteParam.Vent_Mode = 7;
			ctrlWriteParam.Pat_TrigMode = 1;
			ctrlWriteParam.Set_PEEP = 5;
			ctrlWriteParam.Set_RiseTime = 10; 
			ctrlWriteParam.Set_pressuresupport =8; 
			ctrlWriteParam.Set_FI02 = 21;
...
{	
	if(ctrlWriteRead.Vent_Mode == 1 || ctrlWriteRead.Vent_Mode == 10)
	{	
	...
	{
		if(prvcVentSettings.BreathCnt < 2)
				flow_control();//Air and Oxygen mixing function
		else if(prvcVentSettings.BreathCnt >= 2)
			PID_PressureControl_Loop(); // Pressure control PID function calling
	}
...
{	
	...
	else if (simv_pc_ps == 2 || ctrlWriteRead.Vent_Mode == 3 || ctrlWriteRead.Vent_Mode == 7)
...
//checking table for the PID coefficients
				...
				else if(Resistance >= 15 && Resistance <=25)
				{			
...
{						
		...
		DAC_Output_Set(&DAC1,DAC_CHANNEL_1,trigger_dac,LTC2607_WR_AND_UPDATE); // AIR PV SET Function
			...
			{			
					GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_0, GPIO_PIN_0); //Energizing free breathing valve.
...
void comp_calculation()
{
	...
	if(Tid_Vol > 0.3*Insp_Tidal_vol && Volume_read == 0)
	{		
...
void PEEP_control()
{
	...
	{	
		...
		Insp_Tidal_vol = ctrlReadParam.Volume;
		
		ventAlgParam.Pplatue=ctrlReadParam.Insp_Pressure;
		
		for(int index =0 ; index < 50;index++)
		{
			AVG_PEEP_ACC += PEEP_BUFFER[index];
		}
		
		AVG_PEEP = AVG_PEEP_ACC/50;										//calculating average peep value
		
		E_Error = SET_PEEP - AVG_PEEP;											//measuring peep error
		
		AVG_ERROR = fabs(E_Error);
		
		if(AVG_ERROR > 2 && ctrlWriteParam.Pat_TrigMode != 1)peep_error = 0;
		...
		if(peep_error == 0 || SET_PEEP != ctrlWriteRead.Set_PEEP )  //only for first breath or whwn change in set peep
		{
			...
			//testing
			if(SET_PEEP < ctrlWriteRead.Set_PEEP)
					open_per = 0.5;
			//testing
...
//		AVG_PEEP=0;
	...
	PEEP_BUFFER[AVG_CNT++] = ctrlReadParam.Insp_Pressure;
	
	if(AVG_CNT == 50)AVG_CNT=0;
