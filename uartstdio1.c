...
// disable echo by calling UARTEchoSet1(false).
//
//*****************************************************************************
static bool g_bDisableEcho1;

//*****************************************************************************
//
// Output ring buffer.  Buffer is full if g_ui32UARTTxReadIndex1 is one ahead of
// g_ui32UARTTxWriteIndex1.  Buffer is empty if the two indices are the same.
//
//*****************************************************************************
 unsigned char g_pcUARTTxBuffer1[UART_TX_BUFFER_SIZE1];
 volatile uint32_t g_ui32UARTTxWriteIndex1 = 0;
 volatile uint32_t g_ui32UARTTxReadIndex1 = 0;

//*****************************************************************************
//
// Input ring buffer.  Buffer is full if g_ui32UARTTxReadIndex1 is one ahead of
// g_ui32UARTTxWriteIndex1.  Buffer is empty if the two indices are the same.
//
//*****************************************************************************
 unsigned char g_pcUARTRxBuffer1[UART_RX_BUFFER_SIZE1];
 volatile uint32_t g_ui32UARTRxWriteIndex1 = 0;
 volatile uint32_t g_ui32UARTRxReadIndex1 = 0;
...
//
//*****************************************************************************
#define TX_BUFFER_USED1          (GetBufferCount1(&g_ui32UARTTxReadIndex1,  \
                                                &g_ui32UARTTxWriteIndex1, \
                                                UART_TX_BUFFER_SIZE1))
#define TX_BUFFER_FREE1          (UART_TX_BUFFER_SIZE1 - TX_BUFFER_USED1)
#define TX_BUFFER_EMPTY1         (IsBufferEmpty1(&g_ui32UARTTxReadIndex1,   \
                                               &g_ui32UARTTxWriteIndex1))
#define TX_BUFFER_FULL1          (IsBufferFull1(&g_ui32UARTTxReadIndex1,  \
                                              &g_ui32UARTTxWriteIndex1, \
                                              UART_TX_BUFFER_SIZE1))
#define ADVANCE_TX_BUFFER_INDEX1(Index) \
                                (Index) = ((Index) + 1) % UART_TX_BUFFER_SIZE1
...
//
//*****************************************************************************
#define RX_BUFFER_USED1          (GetBufferCount1(&g_ui32UARTRxReadIndex1,  \
                                                &g_ui32UARTRxWriteIndex1, \
                                                UART_RX_BUFFER_SIZE1))
#define RX_BUFFER_FREE1          (UART_RX_BUFFER_SIZE1 - RX_BUFFER_USED1)
#define RX_BUFFER_EMPTY1         (IsBufferEmpty1(&g_ui32UARTRxReadIndex1,   \
                                               &g_ui32UARTRxWriteIndex1))
#define RX_BUFFER_FULL1          (IsBufferFull1(&g_ui32UARTRxReadIndex1,  \
                                              &g_ui32UARTRxWriteIndex1, \
                                              UART_RX_BUFFER_SIZE1))
#define ADVANCE_RX_BUFFER_INDEX1(Index) \
                                (Index) = ((Index) + 1) % UART_RX_BUFFER_SIZE1
#endif
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTBase1[3] =
{
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTInt1[3] =
{
...
//
//*****************************************************************************
static uint32_t g_ui32PortNum1;
#endif
...
//
//*****************************************************************************
static const uint32_t g_ui32UARTPeriph1[3] =
{
...
#ifdef UART_BUFFERED1
static bool
IsBufferFull1(volatile uint32_t *pui32Read,
...
#ifdef UART_BUFFERED1
static bool
IsBufferEmpty1(volatile uint32_t *pui32Read,
...
#ifdef UART_BUFFERED1
static uint32_t
GetBufferCount1(volatile uint32_t *pui32Read,
               volatile uint32_t *pui32Write, uint32_t ui32Size)
{
    uint32_t ui32Write;
    uint32_t ui32Read;
...
UARTPrimeTransmit(uint32_t ui32Base)
{
    ...
    if(!TX_BUFFER_EMPTY1)
    {
        ...
        //
        MAP_IntDisable(g_ui32UARTInt1[g_ui32PortNum1]);

        //
        // Yes - take some characters out of the transmit buffer and feed
        // them to the UART transmit FIFO.
        //
        while(MAP_UARTSpaceAvail(ui32Base) && !TX_BUFFER_EMPTY1)
        {
            MAP_UARTCharPutNonBlocking(ui32Base,
                                      g_pcUARTTxBuffer1[g_ui32UARTTxReadIndex1]);
            ADVANCE_TX_BUFFER_INDEX1(g_ui32UARTTxReadIndex1);
        }

        //
        // Reenable the UART interrupt.
        //
        MAP_IntEnable(g_ui32UARTInt1[g_ui32PortNum1]);
    }
}
#endif
...
//! functions: UARTprintf1() or UARTgets1().  This function assumes that the
...
//*****************************************************************************
void
UARTStdioConfig1(uint32_t ui32PortNum, uint32_t ui32Baud, uint32_t ui32SrcClock)
{
    //
    // Check the arguments.
    //
    ...
    //
    ASSERT(g_ui32Base1 == 0);
#endif

    //
    // Check to make sure the UART peripheral is present.
    //
    if(!MAP_SysCtlPeripheralPresent(g_ui32UARTPeriph1[ui32PortNum]))
    {
        return;
    }
    ...
    //
    g_ui32Base1 = g_ui32UARTBase1[ui32PortNum];

    //
    // Enable the UART peripheral for use.
    //
    MAP_SysCtlPeripheralEnable(g_ui32UARTPeriph1[ui32PortNum]);

    //
    // Configure the UART for 115200, n, 8, 1
    //
    MAP_UARTConfigSetExpClk(g_ui32Base1, ui32SrcClock, ui32Baud,
    ...
    //
    MAP_UARTFIFOLevelSet(g_ui32Base1, UART_FIFO_TX1_8, UART_FIFO_RX1_8);
    ...
    //
    MAP_UARTIntDisable(g_ui32Base1, 0xFFFFFFFF);
    MAP_UARTIntEnable(g_ui32Base1, UART_INT_RX | UART_INT_RT);
    MAP_IntEnable(g_ui32UARTInt1[ui32PortNum]);
#endif

    //
    // Enable the UART operation.
    //
    MAP_UARTEnable(g_ui32Base1);
}
...
//*****************************************************************************
int
UARTwrite1(const char *pcBuf, uint32_t ui32Len)
{
#ifdef UART_BUFFERED1
    unsigned int uIdx;
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(pcBuf[uIdx] == '\n')
        {
            if(!TX_BUFFER_FULL1)
            {
                g_pcUARTTxBuffer1[g_ui32UARTTxWriteIndex1] = '\r';
                ADVANCE_TX_BUFFER_INDEX1(g_ui32UARTTxWriteIndex1);
            }
            else
            {
                //
        ...
        if(!TX_BUFFER_FULL1)
        {
            g_pcUARTTxBuffer1[g_ui32UARTTxWriteIndex1] = pcBuf[uIdx];
            ADVANCE_TX_BUFFER_INDEX1(g_ui32UARTTxWriteIndex1);
        }
        else
        {
    ...
    if(!TX_BUFFER_EMPTY1)
    {
        UARTPrimeTransmit(g_ui32Base1);
        MAP_UARTIntEnable(g_ui32Base1, UART_INT_TX);
    }
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(pcBuf[uIdx] == '\n')
        {
            MAP_UARTCharPut(g_ui32Base1, '\r');
        }

        //
        // Send the character to the UART output.
        //
        MAP_UARTCharPut(g_ui32Base1, pcBuf[uIdx]);
    }
...
//! in buffered mode, a call to UARTPeek1() may be made to determine whether
...
//*****************************************************************************
int
UARTgets1(char *pcBuf, uint32_t ui32Len)
{
#ifdef UART_BUFFERED1
    ...
    while(1)
    {
        ...
        if(!RX_BUFFER_EMPTY1)
        {
            cChar = g_pcUARTRxBuffer1[g_ui32UARTRxReadIndex1];
            ADVANCE_RX_BUFFER_INDEX1(g_ui32UARTRxReadIndex1);
    ...
    while(1)
    {
        ...
        //
        cChar = MAP_UARTCharGet(g_ui32Base1);
        ...
        if(ui32Count < ui32Len)
        {
            ...
            //
            MAP_UARTCharPut(g_ui32Base1, cChar);
        }
    }
...
UARTgetc1(void)
{
    ...
    // the caller.
    //
    return(MAP_UARTCharGet(g_ui32Base1));
#endif
}
...
//*****************************************************************************
void
UARTvprintf1(const char *pcString, va_list vaArgP)
{
...
convert:
                    ...
                    {
                        pcBuf[ui32Pos++] =
                            g_pcHex1[(ui32Value / ui32Idx) % ui32Base];
                    }

                    //
                    // Write the string.
                    //
                    UARTwrite1(pcBuf, ui32Pos);
...
//*****************************************************************************
void
UARTprintf1(const char *pcString, ...)
{
    va_list vaArgP;
...
//! buffered mode using \b UART_BUFFERED1, may be used to determine the number
//! of bytes of data currently available in the receive buffer.
//!
//! \return Returns the number of available bytes.
//
//*****************************************************************************
#if defined(UART_BUFFERED1) || defined(DOXYGEN)
int
UARTRxBytesAvail1(void)
{
    return(RX_BUFFER_USED1);
}
#endif
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED1, may be used to determine the amount
...
//*****************************************************************************
int
UARTTxBytesFree1(void)
{
    return(TX_BUFFER_FREE1);
}
#endif
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED1, may be used to look ahead in the
...
#if defined(UART_BUFFERED1) || defined(DOXYGEN)
int
UARTPeek1(unsigned char ucChar)
{
    int iCount;
    int iAvail;
    ...
    //
    iAvail = (int)RX_BUFFER_USED1;
    ui32ReadIndex = g_ui32UARTRxReadIndex1;
    ...
    {
        if(g_pcUARTRxBuffer1[ui32ReadIndex] == ucChar)
        {
            //
        ...
        else
        {
            ...
            //
            ADVANCE_RX_BUFFER_INDEX1(ui32ReadIndex);
        }
    }
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED1, may be used to discard any data
//! received from the UART but not yet read using UARTgets1().
//!
//! \return None.
//
//*****************************************************************************
#if defined(UART_BUFFERED1) || defined(DOXYGEN)
void UARTFlushRx1(void)
{
    uint32_t ui32Int;
    ...
    //
    g_ui32UARTRxReadIndex1 = 0;
    g_ui32UARTRxWriteIndex1 = 0;
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED1, may be used to flush the transmit
//! buffer, either discarding or transmitting any data received via calls to
//! UARTprintf1() that is waiting to be transmitted.  On return, the transmit
...
//*****************************************************************************
#if defined(UART_BUFFERED1) || defined(DOXYGEN)
void UARTFlushTx1(bool bDiscard)
{
    uint32_t ui32Int;
    ...
    if(bDiscard)
    {
        ...
        //
        g_ui32UARTTxReadIndex1 = 0;
        g_ui32UARTTxWriteIndex1 = 0;
...
//!
//! This function, available only when the module is built to operate in
//! buffered mode using \b UART_BUFFERED1, may be used to control whether or not
...
#if defined(UART_BUFFERED1) || defined(DOXYGEN)
void
UARTEchoSet1(bool bEnable)
{
    g_bDisableEcho1 = !bEnable;
}
#endif
...
UARTStdioIntHandler1(void)
{
    ...
    //
    ui32Ints = MAP_UARTIntStatus(g_ui32Base1, true);
    MAP_UARTIntClear(g_ui32Base1, ui32Ints);
    ...
    if(ui32Ints & UART_INT_TX)
    {
        ...
        if(TX_BUFFER_EMPTY1)
        {
            MAP_UARTIntDisable(g_ui32Base1, UART_INT_TX);
        }
    }
    ...
    if(ui32Ints & (UART_INT_RX | UART_INT_RT))
    {
        ...
        //
        while(MAP_UARTCharsAvail(g_ui32Base1))
        {
            //
            // Read a character
            //
            i32Char = MAP_UARTCharGetNonBlocking(g_ui32Base1);
	...
	/*			if(!g_bDisableEcho1)
            {
                ...
                if(cChar == '\b')
                {
                    ...
                    if(!RX_BUFFER_EMPTY1)
                    {
                        ...
                        // Decrement the number of characters in the buffer.
                        //
                        if(g_ui32UARTRxWriteIndex1 == 0)
                        {
                            g_ui32UARTRxWriteIndex1 = UART_RX_BUFFER_SIZE1 - 1;
                        }
                        else
                        {
                            g_ui32UARTRxWriteIndex1--;
                ...
                if((cChar == '\r') || (cChar == '\n') || (cChar == 0x1b))
                {
                    ...
                    // UARTgets1() where the line ends.  We also send an
...
*/
            ...
            if(!RX_BUFFER_FULL1)
            {
                ...
                //
                g_pcUARTRxBuffer1[g_ui32UARTRxWriteIndex1] =
                    (unsigned char)(i32Char & 0xFF);
                ADVANCE_RX_BUFFER_INDEX1(g_ui32UARTRxWriteIndex1);
