...
//---[ Macros ]-----------------------------------------------------------------
#define VCV_BREATHS   2
#define PRESSURE_DIF  5
#define P_VOL_ERROR_lIMIT  10
#define N_VOL_ERROR_lIMIT  -10
//---[ Constants ]--------------------------------------------------------------


//---[ Types ]------------------------------------------------------------------
 typedef struct _prvcsettings
 {
    uint8_t BreathCnt;
    bool  ChangeFlag;
    float VolError;
    float PmaxPressureError;
    float Set_pmax;
    float Set_TidalVolume;
    float PRVC_SetPressure;
		float complaince;
		float errctrlCnst;
 }prvcsettings;
...
extern uint8_t peep_delay,Peep_Flag;
extern  float Flow_trigger_offset;
extern float Max_E_Tidal_vol,Volume_read,Insp_Tidal_vol_A;
 
extern float prvcAvgComplainceArray[5] , prvcAvgComplaince;
extern uint8_t prvcINDEX;


//---[ Public Function Prototypes ]---------------------------------------------
extern void VolCtrl_Algorithm(void);
extern void PressureCtrl_Algorithm(void);
extern prvcsettings prvcVentSettings;
void PRVC(void);
#endif
